...
class CodeExecutor:
    ...
    def __init__(self, timeout: int = 30, max_workers: int = 10):
        ...
        self.thread_pool = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="code_executor")
    ...
    async def execute(self, code: str, language: str = "python3") -> Dict[str, Any]:
        try:
            ...
            if language == "python3":
                ...
                logging.error(f"不支持的语言: {language}")
                return {
                    "success": False,
                    "output": "",
...
code_executor = CodeExecutor(timeout=app_config.WORKER_TIMEOUT, max_workers=app_config.MAX_WORKERS)
